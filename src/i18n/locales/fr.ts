import { TranslationDictionary } from "../types";

export const fr: TranslationDictionary = {
  app: {
    name: "Taleno",
    tagline: "Lecteur et éditeur Markdown minimaliste et fluide",
    description: "Un lecteur et éditeur Markdown WYSIWYG local-first minimaliste conçu avec Tauri 2, Rust et SolidJS.",
    copyright: "Copyright © 2026 Taleno Contributors",
    licenseNotice: "Licence GNU AGPL-3.0",
  },
  menu: {
    file: "Fichier",
    edit: "Édition",
    view: "Affichage",
    window: "Fenêtre",
    help: "Aide",
    preferences: "Préférences",
    plugins: "Extensions",
    pluginsQuick: "Extensions (Ctrl+Maj+X)",
    themesQuick: "Thèmes (Ctrl+,)",
    languageQuick: "Langue",
    language: "Langue (Language)",
    languageSubtitle: "Sélectionner la langue d'affichage",
    autoLanguage: "Automatique (système)",
    autoLanguageDesc: "Détection automatique de la langue (Actuel : {current})",
  },
  file: {
    newDocument: "Nouveau document",
    openFile: "Ouvrir un fichier...",
    openFolder: "Ouvrir un dossier...",
    save: "Enregistrer",
    saveAs: "Enregistrer sous...",
    closeTab: "Fermer l'onglet",
    exportAs: "Exporter sous",
    exportHtml: "Exporter en HTML (.html)",
    exportPdf: "Exporter en PDF (.pdf)",
    exit: "Quitter",
  },
  edit: {
    undo: "Annuler",
    redo: "Rétablir",
    cut: "Couper",
    copy: "Copier",
    paste: "Coller",
    selectAll: "Tout sélectionner",
    findReplace: "Rechercher et remplacer",
    bold: "Gras",
    italic: "Italique",
    strikethrough: "Barré",
    heading: "Titre",
    heading1: "Titre 1",
    heading2: "Titre 2",
    heading3: "Titre 3",
    heading4: "Titre 4",
    heading5: "Titre 5",
    heading6: "Titre 6",
    paragraph: "Paragraphe / Texte normal",
    bulletList: "Liste à puces",
    numberedList: "Liste numérotée",
    taskList: "Liste de tâches",
    codeBlock: "Bloc de code",
    blockquote: "Citation",
    table: "Tableau",
    link: "Insérer un lien",
    image: "Insérer une image",
  },
  view: {
    toggleSidebar: "Basculer la barre latérale",
    toggleOutline: "Basculer le plan",
    quickSwitcher: "Sélecteur rapide",
    readingMode: "Mode lecture",
    writingMode: "Mode écriture",
    codeMode: "Mode code source",
    lightTheme: "Thème clair",
    darkTheme: "Thème sombre",
    systemTheme: "Suivre le thème du système",
    themeSettings: "Thème...",
  },
  settings: {
    title: "Paramètres",
    themeTab: "Thème",
    pluginsTab: "Extensions",
    updatesTab: "Mises à jour",
    markdownTheme: "Thème",
    markdownThemeDescription: "Choisissez le style visuel de Taleno et du Markdown rendu.",
    colorMode: "Mode de couleur",
    colorModeDescription: "Les modes clair, sombre et système s’appliquent à tous les thèmes.",
    elementShadows: "Activer les ombres des éléments",
    elementShadowsDescription: "Ajouter des ombres discrètes aux images et tableaux rendus.",
    automaticUpdateChecks: "Rechercher automatiquement les mises à jour",
    automaticUpdateChecksDescription: "Vérifier le canal stable une fois par jour et vous prévenir avant l’installation.",
    stableChannelDescription: "Les mises à jour ne sont téléchargées et installées qu’après votre confirmation.",
    pluginsDescription: "Enrichissez Taleno avec des outils communautaires et des fonctionnalités d'édition.",
    searchPlugins: "Rechercher des extensions...",
    openPluginsFolder: "Ouvrir le dossier des extensions",
    reloadPlugins: "Recharger les extensions",
    noPluginsFound: "Aucune extension ne correspond à votre recherche.",
    noPluginsInstalled: "Aucune extension installée pour le moment.",
    pluginsDirectoryTip: "Déposez vos dossiers d'extension dans le répertoire pour les installer.",
    pluginAuthor: "Auteur",
    pluginVersion: "Version",
    pluginEnabled: "Activé",
    pluginDisabled: "Désactivé",
    installedTab: "Installées",
    marketplaceTab: "Catalogue",
    installPlugin: "Installer",
    installing: "Installation...",
    uninstallPlugin: "Désinstaller",
    uninstalling: "Suppression...",
    updatePlugin: "Mettre à jour",
    updating: "Mise à jour...",
    updateAvailable: "Mise à jour disponible",
    installedBadge: "Installée",
    searchMarketplace: "Rechercher des extensions officielles et communautaires...",
    noMarketplacePlugins: "Aucune extension trouvée.",
    marketplaceOffline: "Connexion au registre impossible. Affichage du cache local.",
    retryMarketplace: "Réessayer",
    customThemes: "Thèmes personnalisés et communautaires",
    customThemesDescription: "Personnalisez votre éditeur avec des styles CSS conçus par la communauté.",
    builtInThemes: "Thèmes intégrés",
    openThemesFolder: "Ouvrir le dossier des thèmes",
    activeTheme: "Actif",
    applyTheme: "Appliquer",
    appliedTheme: "Appliqué",
    defaultTheme: "Par défaut",
    noCustomThemesInstalled: "Aucun thème personnalisé installé pour le moment.",
    searchThemes: "Rechercher des thèmes...",
    noThemesFound: "Aucun thème ne correspond à votre recherche.",
    installTheme: "Installer",
    uninstallTheme: "Désinstaller",
  },
  window: {
    minimize: "Réduire",
    maximize: "Agrandir",
    restore: "Restaurer",
    close: "Fermer",
  },
  help: {
    documentation: "Documentation",
    github: "Dépôt GitHub",
    website: "Site officiel",
    checkForUpdates: "Rechercher des mises à jour...",
    reportIssue: "Signaler un problème",
    about: "À propos de Taleno",
  },
  welcome: {
    title: "Taleno",
    subtitle: "Lecteur et éditeur Markdown minimaliste et fluide",
    newDocument: "Nouveau document",
    openFile: "Ouvrir un fichier",
    github: "GitHub",
    recentFiles: "Fichiers récents",
    noRecentFiles: "Aucun fichier récent. Glissez-déposez un fichier pour commencer.",
    shortcuts: "Raccourcis clavier",
    tip: "Astuce : Glissez-déposez un fichier Markdown directement dans cette fenêtre pour l'ouvrir",
    dblClickHint: "Double-cliquez sur une zone vide pour créer un nouveau document",
  },
  sidebar: {
    outline: "Plan",
    files: "Fichiers",
    workspace: "Espace de travail",
    sectionsCount: "sections",
    noHeadings: "Aucun titre dans le document actuel.",
    noFolderOpen: "Aucun dossier ouvert. Cliquez ci-dessous pour ouvrir un espace de travail.",
    openFolder: "Ouvrir un espace de travail",
    searchFiles: "Rechercher des fichiers...",
    newFile: "Nouveau fichier",
    newFolder: "Nouveau dossier",
    rename: "Renommer",
    delete: "Supprimer",
    confirmDelete: "Voulez-vous vraiment supprimer ce fichier ?",
  },
  statusBar: {
    wordsCount: "mots",
    lineCol: "Lig {line}, Col {col}",
    reading: "Lecture",
    writing: "Écriture",
    code: "Code",
    encoding: "UTF-8",
    format: "Markdown",
    toggleTheme: "Changer de thème",
    checkingUpdates: "Vérification...",
    upToDate: "À jour",
    updateAvailable: "Mise à jour dispo",
  },
  update: {
    checking: "Recherche de mises à jour...",
    checkingDesc: "Connexion au canal de mise à jour stable. Cela peut prendre un moment.",
    localizedReleaseNotes: "La version {version} comprend les dernières fonctionnalités, des améliorations de compatibilité et des corrections de bugs.",
    upToDate: "Vous utilisez la dernière version stable",
    upToDateDesc: "Votre version est la version stable la plus récente.",
    aheadTitle: "🌟 En avance sur la version publique",
    aheadDesc: "La version locale {current} est en avance sur la dernière version publique {latest}.",
    aheadBadge: "Version préliminaire",
    updateAvailable: "Nouvelle version disponible !",
    latestVersion: "Dernière version",
    currentVersion: "Version actuelle",
    downloadUpdate: "Télécharger la mise à jour",
    remindLater: "Me rappeler plus tard",
    releaseNotes: "Notes de version",
    downloading: "Téléchargement de la mise à jour… {progress}%",
    installing: "Installation de la mise à jour…",
    installAndRestart: "Installer et redémarrer",
    retry: "Réessayer",
    updateErrorTitle: "Échec de la mise à jour",
    updateErrorDesc: "Taleno n’a pas pu télécharger ou installer la mise à jour. Veuillez réessayer.",
    unsavedTitle: "Documents non enregistrés",
    unsavedMessage: "L’installation fermera Taleno. Enregistrez d’abord vos documents ou choisissez explicitement de continuer sans enregistrer.",
    discardAndInstall: "Ignorer et installer",
  },
  dialogs: {
    ok: "OK",
    cancel: "Annuler",
    close: "Fermer",
    delete: "Supprimer",
    save: "Enregistrer",
    dontSave: "Ne pas enregistrer",
  },
};
